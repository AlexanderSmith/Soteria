...
public class GameDirector: MonoBehaviour {
	...
	void GenerateTiles()
	{
		if (LevelWidth > 0 && LevelHeight > 0)
		{
			for (int y = 0; y < LevelHeight; y++)
			{
				...
				{
					Texture2D text = null;
					Rect TexRec = new Rect(x * tilewidth, y * tileheight, tilewidth, tileheight);
	...
	void spawnEnemy()
	{
		...
		go.SetActive(true);
		go.transform.position = Vector3.zero;
		go.transform.Translate((currentTile.x + 1)* tilewidth - 16.0f, 0.0f, (currentTile.y + 1) * tileheight - 20.0f);
