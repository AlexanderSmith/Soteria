...
public class EyeballShadowCreatureSpawner : MonoBehaviour
{
	...
	private void ShadowCreatureSpawner()
	{
		...
		GameObject enemy = Instantiate (enemyPrefab, spawnLocations [_spawnerIndex].position, spawnLocations [_spawnerIndex].rotation) as GameObject;	enemy.GetComponent<BasicEnemyController>().GetAgent().SetDestination(GameDirector.instance.GetPlayer().transform.position);
	}
